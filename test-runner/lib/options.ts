...
export const options = getopts(process.argv.slice(2), optParams);

export function formatUsage(configMode: string): string {
    const styleModeText = configMode === "styleMode"
        ? " (-S MUST be used in this configuration)"
        : "";
    return "Usage: " + path.basename(process.argv[1]) + " <-s testName|-g groupName|-a|-l|-V all/country/filename> [-S styleName|-w cslFilePath|-C cslJsonFilePath]\n\n"
